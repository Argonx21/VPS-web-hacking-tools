...
unset OS
	...
	cd /tmp && https://go.dev/dl/go${GOVER}.linux-amd64.tar.gz > /dev/null 2>&1 && tar xvf go${GOVER}.linux-amd64.tar.gz > /dev/null 2>&1 && mv go /usr/local && echo "export PATH=$PATH:/usr/local/go/bin" >> ~/.bashrc && source ~/.bashrc;
